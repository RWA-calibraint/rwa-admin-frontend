...
import { CustomText } from '@/helpers/interface';
import textcss from '@components/text-editor/text.module.scss';
...
const Toolbar = ({ editor }: { editor: Editor }) => {
  ...
  return (
    <div
      className="d-flex gap-2 border-primary-1 editor radius-8-8-0-0"
      onMouseDown={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
...
const SlateEditor = ({ value, onChange }: { value: Descendant[]; onChange: (value: Descendant[]) => void }) => {
  ...
  return (
    <Slate editor={editor} initialValue={slateValue} onChange={handleChange}>
      ...
      <div className="p-8 border-primary-1 radius-0-0-8-8 border-top-none">
        <Editable
          key="editable"
      ...
      <div className="d-flex justify-flex-end m-t-8">
        {<p className="f-14-20-400-failure">{error}</p>}
