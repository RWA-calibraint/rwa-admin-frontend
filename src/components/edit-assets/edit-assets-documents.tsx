import '@/app/asset-edit/styles.scss';
import { Button, Form, FormInstance, Upload, UploadFile } from 'antd';
import { RcFile } from 'antd/es/upload';
import { Check, FileImage, FileText, LoaderCircle, Trash2, UploadIcon, X } from 'lucide-react';
import { usePathname } from 'next/navigation';
import Script from 'next/script';
import React, { useCallback, useEffect, useRef } from 'react';

import { UploadDocInterface } from '@/app/asset-edit/edit.interface';
...
import { ASSET_DOCUMENTS } from './constants';
...
const EditAssetDocument = ({
  ...
  const handleUploadSuccess = useCallback(
    async (key: string, newFile: UploadFile, fileList: UploadFile[]) => {
      try {
        ...
        updateFileStatus(key, fileList, newFile.uid, {
          uploadedS3Url: response.response.documentS3URL,
          isError: false,
        });
      } catch (error) {
  ...
  const handleBeforeUpload = useCallback(async (newFile: RcFile, fileList: RcFile[]) => {
    try {
      ...
      if (!newFile.name.trim() && isDuplicateFile(newFile, fileList)) {
        showErrorToast(new Error(ERROR_MESSAGE.FILE_NAME_INVALID));
  ...
  return (
    <div
      className={`w-662 border-none-0 radius-12 bg-white overflow-auto p-x-24 ${
        pathname.includes('/create-asset') ? 'v-h-65' : 'v-h-72'
      }`}
    >
      <Script
      ...
      />
      <CustomAlertBox>
        <strong>Supported file format:</strong> png, jpeg, pdf / <strong>Max file size:</strong> 50 MB
      </CustomAlertBox>
            ...
            <Upload
              ...
              itemRender={(_, file) => (
                <div
                  ...
                  style={{ border: '1px solid #E4E4E7', borderRadius: '8px', padding: '10px' }}
                >
                  ...
                  <span className="text-sm font-medium flex-1 text-center">{file?.name ?? 'filename'}</span>
                  <Trash2
                    size={16}
                    color="gray"
