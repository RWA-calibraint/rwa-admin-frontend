...
const EditAssetHistory: FC<EditAssetHistoryProps> = ({ form, rows, setRows }) => {
  ...
  const disabledDate = (current: dayjs.Dayjs | null) => {
    ...
    return selectedYears.includes(String(year)) || year > currentYear || year < minYear || year > maxYear;
  };
  ...
  return (
    ...
    >
      {rows.map((row) => (
        <div key={row.key} className="d-flex align-start width-100">
          <Form.Item
            label={
              <span>
          ...
          <Form.Item
            ...
            rules={[
              ...
              {
                validator: (_, value) =>
                  value > 1 ? Promise.resolve() : Promise.reject(new Error('Price must be greater than 1')),
              },
            ]}
          ...
          <div className="d-flex align-center m-t-8" style={{ width: '50px' }}>
            {row.key !== 1 && <Trash2 onClick={() => removeRow(row.key)} className="img-20 cursor-pointer m-l-8" />}
          </div>
        </div>
      ))}
    </div>
