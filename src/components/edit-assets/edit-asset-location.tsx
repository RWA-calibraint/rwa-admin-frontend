...
const EditAssetLocation = ({
  ...
  const { isLoaded, loadError } = useLoadScript({
    googleMapsApiKey: ENV_CONFIGS.GOOGLE_API_KEY || 'AIzaSyD_FiHj4eww6971nehQJ9cdI5fCMjrX1NQ',
    libraries,
  });
  ...
  return (
    <div
      className={`w-662 border-none-0 radius-12 bg-white overflow-auto p-x-24 ${pathName.includes('/create-asset') ? 'v-h-65' : 'v-h-72'}`}
    >
      {!isLoaded ? (
        <Skeleton active paragraph={{ rows: 4 }} />
      ) : (
        <div className="asset-location-form-container">
          <Form.Item
            label={
